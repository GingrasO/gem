...
from triqs_ghostGA.utils_TH import get_semicircle_e_list        
...
if __name__ == "__main__":                                                         
    ...
    #Build quartic part                                                            
    Utype = "Kanamori"    #"Kanamori" or "Slater"                                    
    if Utype == "Kanamori":                                                        
        try:                                                                       
            Utensor = Uijkl(nimp//2, U_int=U, J_hund=J, full_Uijkl=True)           
            assert Utensor.shape==(nimp//2,nimp//2,nimp//2,nimp//2)
        finally:                                                                   
            print("Kanamori UTensor construction breaks. The 4-index functionality is only present in triqs version >= 3.2.")
    elif Utype == "Slater":                                                        
        Utensor = U_matrix_slater((nimp//2 -1)//2, U_int=U, J_hund=J)                      
        assert Utensor.shape==(nimp//2,nimp//2,nimp//2,nimp//2)                                                                               
                                                                                   
    #Build quadratic part                                                          
    eloc_spinless = np.eye(nimp//2)*(-mu)#np.zeros((nimp//2, nimp//2))                                   
    ...
    Lambda0 = promote_to_spinful(Lambda0,Lambda0)                                        
    ...
    grisb.run(itmax=100, mix=0.5, tol=1e-5, beta=500, silence=False, spin_pen=0.05)
    t2 = time.time()                                                               
