...
U_list = np.array([0.5]) #np.array([0.5,2.0,3.5])
T_list = np.linspace( 2e-2, 1, 50)

# mag_grid[iU, iT] = |m_A|, order parameter on sublattice A
mag_grid = np.zeros((len(U_list), len(T_list)))

h5_file = 'data_Square_1orb_2frag_phase.h5'

# create a fresh file; each U group will be appended as it completes
with h5py.File(h5_file, 'w') as _: # 'a' is append, 'w' is write if you want to delete previous
    pass

L_t0 = np.kron( np.diag([0.0,-0.7,0.7]) , np.eye(2) ) 
#adding a small magnetization to stabilize AFM
L_t0 -= bfield*np.kron( np.eye(B), np.diag([1.0,-1.0]) )
R_t0 = np.kron( np.array([[0.8],[0.2],[0.2]]) , np.eye(2) )
...
for iU, U in enumerate(U_list):
    ...
    for iT, T in enumerate(T_list):
        ...
        for it in range(itmax):
            ...
            print('A lambda eigvals:', np.linalg.eigvalsh(fragmentA.Lambda))
            print('B lambda eigvals:', np.linalg.eigvalsh(fragmentB.Lambda))

            fragmentA.update_hybridization(T=T,move_pen=1e-6)
            fragmentB.update_hybridization(T=T,move_pen=1e-6)

            print('A lambda_c eigvals:', np.linalg.eigvalsh(fragmentA.Lambda_c))
            print('B lambda_c eigvals:', np.linalg.eigvalsh(fragmentB.Lambda_c))
            ...
            fragmentA.update_self_energy(T=T,move_pen=1e-6)
            fragmentB.update_self_energy(T=T,move_pen=1e-6)
            ...
            magA = abs(fragmentA.denMat[0, 0].real - fragmentA.denMat[1, 1].real)
            magB = abs(fragmentB.denMat[0, 0].real - fragmentB.denMat[1, 1].real)

            print('diff=', diff, 'magA=', magA, 'magB=', magB)
            if (diff < tol and it > 3) or it == itmax - 1:
