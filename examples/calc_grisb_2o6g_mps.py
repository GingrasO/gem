...
class TestGrisb(unittest.TestCase):
    def runTest(self):
        ...
        solver = ITensorMPSSolver(ntot, nimp, nbath, params={"use_Sz":True,"use_Ntot":True,"spin_pen":0.0})
        print(solver.type) 
        grisb = Grisb(ntot, nimp, nbath, eks, eloc, Utensor, R=R0, Lambda=Lambda0, edsolver=solver)
        grisb.run(itmax=100, mix=0.5, tol=1e-6, beta=500, silence=True, spin_pen=0.0)
