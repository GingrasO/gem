...
class Fed(object):
    ...
    def build_operators(self):
        ...
        for i in range(self.ntot//2):
            Sp += self.FH_list[2*i].dot(self.FH_list[2*i+1].H)
        #build total spin operators S-
        Sm = sparse.csr_matrix((self.hsize,self.hsize), dtype=numpy.complex128)
        for i in range(self.ntot//2):
            Sm += self.FH_list[2*i+1].dot(self.FH_list[2*i].H)
        self.Sz = sparse.csr_matrix((self.hsize,self.hsize), dtype=numpy.complex128)
        for i in range(self.ntot//2):
            self.Sz += (0.5*self.FH_list[2*i].dot(self.FH_list[2*i].H) - 0.5*self.FH_list[2*i+1].dot(self.FH_list[2*i+1].H) )
        #build S^2 operator
    ...
    def build_Hemb(self, D, H1E, LAMBDA, V2E, spin_pen=0.0):
        ...
        for i in range(self.nimp):
            for j in range(self.nimp):
                self.Hemb += H1E[i,j]*self.FH_list[i].dot(self.FH_list[j].H)
        #build hybridization part
        for i in range(self.nbath):
            for j in range(self.nimp):
                self.Hemb += D[i,j]*self.FH_list[j].dot(self.FH_list[self.nimp+i].H )
                self.Hemb += D[i,j].conj()*self.FH_list[self.nimp+i].dot(self.FH_list[j].H )
        #build bath part
        for i in range(self.nbath):
            for j in range(self.nbath):
                self.Hemb += LAMBDA[i,j]*self.FH_list[j+self.nimp].H.dot(self.FH_list[i+self.nimp] )
                ...
                for k in range(self.nimp):
                    for l in range(self.nimp):
                        if numpy.abs(V2E[i,k,j,l]) > 1e-6:
                            self.Hemb += 0.5*V2E[i,k,j,l]*self.FH_list[i].dot(self.FH_list[j]).dot(self.FH_list[l].H).dot(self.FH_list[k].H)
                            self.H2e += 0.5*V2E[i,k,j,l]*self.FH_list[i].dot(self.FH_list[j]).dot(self.FH_list[l].H).dot(self.FH_list[k].H)
        #add spin penalty:
        self.Hemb += spin_pen*self.S2
        #assert(numpy.allclose(Hemb.todense(),Hemb.H.todense()))
        return self.Hemb
    ...
    def solve_Hemb(self,num_eig=10, verbose=0):
        ...
        while numpy.abs(self.e0 - self.evals[it]) < 1e-5 and it<(num_eig-1):
            self.deg += 1
            it += 1
        ...
        for i in range(num_eig):
            print(self.evals[i], self.evecs[:,i].conj().T.dot( self.S2.dot( self.evecs[:,i] ) ))
            print('deg=',self.deg)
