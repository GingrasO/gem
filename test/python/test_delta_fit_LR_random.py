...
class TestSelfEnergySolverB3_random_generated(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.Lambda_target = random_hermitian(Bsize, rng, scale=0.1)
        self.R_target = random_complex((Bsize, size), rng, scale=0.1)

        self.x_target = pack_params(self.Lambda_target, self.R_target)

        self.Lambda_c = random_hermitian(Bsize, rng, scale=0.1)
        self.D = random_complex((Bsize, size), rng, scale=0.1)
    ...
    def _perturb_initial_guess(self):
        ...
        Lambda_pert = random_hermitian(Bsize, rng, scale=0.1)
        R_pert = random_complex((Bsize, size), rng, scale=0.1)
